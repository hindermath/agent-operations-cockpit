...
| `AEPS-GAP-AOC-012` | Der generische Gate-Validator erzwingt keine disjunkte Form fuer `Applicable` und `N/A`; nicht anwendbare Gates koennen Ausfuehrungstokens behalten. / The generic gate validator does not enforce disjoint Applicable and N/A shapes, so non-applicable gates may retain execution tokens. | Autonomous Run | Widerspruechliche Evidence kann als gueltig gelten oder spaeter als versteckte Ausfuehrungsanforderung gelesen werden. / Contradictory evidence may pass or later look like a hidden execution requirement. | Positive und negative N/A-Shape-Fixtures auf Bash und PowerShell in einem zweiten Projekt pruefen. | High |
| `AEPS-GAP-AOC-013` | Dauerhafte Completion-Evidence prueft nicht, ob lokale Ergebnisreferenzen nach Ende des Laufs noch aufloesbar sind. / Durable completion evidence does not verify that local result references remain resolvable after the run. | Autonomous Run, moeglicher AEPS Evidence Contract | Ein spaeteres Audit sieht `Pass`, aber nicht mehr die gebundene Ausgabe. / A later audit sees Pass but can no longer inspect the bound output. | Temp-Pfad-, Commit-Blob-, Repository-Hash- und Provider-Artefakt-Fixtures mit Redaktions- und Retention-Grenzen pruefen. | High |
| `AEPS-GAP-AOC-014` | Ein feature-lokaler Lifecycle-Datensatz wird von den generischen Intake-Governance-Konsumenten nicht aufgeloest. / Generic intake-governance consumers do not resolve a feature-local lifecycle record. | Intake Authoring, Intake Review, Intake Sequencing, Autonomous Run | Eine gueltig archivierte akzeptierte Quelle blockiert spaetere Status- und Review-Laeufe mit `RIG014`. / A valid archived accepted source blocks later status and review operations with RIG014. | Portable positive und negative Rename-/Archive-Fixtures gegen alle Intake-Presets und einen terminalen Series-Zustand pruefen. | High |
