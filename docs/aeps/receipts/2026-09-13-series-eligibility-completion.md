...
| `AEPS-FIND-AOC-017` | Kausale Feature-/Lifecycle-Trennung mit zeitgleichen Runtime-Snapshots; der Closeout-Pfad ist vorab deklariert. / Causal feature/lifecycle separation with contemporaneous runtime snapshots; the closeout path is predeclared. | Frühere Läufe verloren temporäre Logs oder mussten PreMerge rekonstruieren; Closeout-Snapshots für diesen Lauf stehen noch aus. / Earlier runs lost temporary logs or reconstructed premerge evidence; this run's closeout snapshots remain pending. | Closeout-Veröffentlichung und unabhängiges Referenzprojekt stehen aus. / Closeout publication and an independent reference project remain pending. |
