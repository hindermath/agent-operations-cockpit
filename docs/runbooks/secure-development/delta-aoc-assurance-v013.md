# delta: AOC-Feldtest / AOC field test
...
genehmigter Governance-Scope ohne Produktlauf. Nur delta prüfen.
...
scope. Review only delta; compare ordinal relative paths and raw SHA-256 of all
...
```bash
bash .specify/presets/secure-development-assurance-governance/scripts/validate-secure-development-assurance.sh review delta aoc-assurance-v013 training
```

```powershell
pwsh -NoProfile -File .specify/presets/secure-development-assurance-governance/scripts/validate-secure-development-assurance.ps1 -Action Review -Gate delta -ContextId aoc-assurance-v013 -Mode training
```
