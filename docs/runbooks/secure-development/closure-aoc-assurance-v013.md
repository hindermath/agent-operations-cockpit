# closure: AOC-Feldtest / AOC field test
...
genehmigter Governance-Scope ohne Produktlauf. Nur closure prüfen.
...
scope. Review only closure; compare ordinal relative paths and raw SHA-256 of all
...
```bash
bash .specify/presets/secure-development-assurance-governance/scripts/validate-secure-development-assurance.sh review closure aoc-assurance-v013 training
```

```powershell
pwsh -NoProfile -File .specify/presets/secure-development-assurance-governance/scripts/validate-secure-development-assurance.ps1 -Action Review -Gate closure -ContextId aoc-assurance-v013 -Mode training
```
