...
Beim damaligen Authoring galt als historischer Snapshot: RAW-01, RAW-03 und
RAW-02 waren `Completed`, RAW-04 war der einzige deklarierte
...
in der [`order.md`](../series/order.md). RAW-03 bleibt der einzige bindende
Vorgänger; Reihenfolge und Abhängigkeiten ändern sich nicht. / *At authoring
time, the historical snapshot recorded RAW-01, RAW-03, and RAW-02 as Completed,
RAW-04 as the sole Eligible candidate, and RAW-05 as limited to read-only
research. This snapshot is not a current lifecycle source. Only the linked
manifest and order document define the current canonical state. RAW-03 remains
the sole binding predecessor; order and dependencies do not change.*
