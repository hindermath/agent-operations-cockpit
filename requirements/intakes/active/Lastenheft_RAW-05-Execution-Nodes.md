...
META-LH-05 sowie RAW-01 bis RAW-04 waren `Completed`, RAW-05 war der einzige
deklarierte `Eligible`-Kandidat und blieb auf read-only Research begrenzt.
...
in der [`order.md`](../series/order.md). RAW-02 → RAW-05 bleibt nur bevorzugte
Reihenfolge. / *At authoring time, the historical snapshot recorded the META
intakes and RAW-01 through RAW-04 as Completed and RAW-05 as the sole Eligible
candidate limited to read-only research. This snapshot is not a current
lifecycle source. Only the linked manifest and order document define the
current canonical state. RAW-02 to RAW-05 remains preferred order only.*
