...
**Status**: Entwurf, fuer die Klaerungsphase bereit / Draft, ready for clarification

**Eingabe / Input**: Exakt gebunden an `requirements/intakes/active/Lastenheft_META-LH-03-Authoring-Contract.md`; keine Produktimplementierung, kein Intake-Update oder -Delete, kein Review-Start, keine nachgelagerte Ausfuehrung, keine Level-0-Aenderung und keine Preset-Promotion. / Exactly bound to `requirements/intakes/active/Lastenheft_META-LH-03-Authoring-Contract.md`; no product implementation, intake update or delete, review start, downstream execution, Level-0 change, or preset promotion.
...
- Lastenheft / requirements intake: SHA-256 `f6d57cacc954b4899fc5bd8ddcc235570ec20470094feec506e1b8e9ea07e3e9`
- Ready-Single-Review: `specs/intake-review-results/meta-lh-03-authoring-contract-2026-08-29-r6.json`, SHA-256 `9de62c488c4223e78636837e78c3d36910f5a11664df45cc6f53c1ffc8d83e5a`, Review-ID `a32cb9e3-a356-4235-9d86-cd1d0efc1b71`, Status `Ready`, ohne Findings, Fragen, akzeptierte Risiken oder Operator-Ausnahmen / without findings, questions, accepted risks, or operator exceptions
- Authoring Receipt: `specs/intake-authoring-receipts/META-LH-03-Authoring-Contract.json`, SHA-256 `6acc207f5533eb9fb92613f23537c669605eef475eb5bb3a911806f6c28e1aeb`
...
**Im Scope / In scope** sind Naming, Pflichtfelder, Provenienz, normalisierte Hashes, Review-Handoff, Prompt-Bindung und Validierung fuer genau einen neuen Intake oder eine ausdruecklich genehmigte atomare Intake-Serie. Die fachliche Liefermenge besteht aus genau den fuenf kanonischen Vertragsartefakten und der im Lastenheft benannten positiven sowie negativen Evidence. / In scope are naming, mandatory fields, provenance, normalized hashes, review handoff, prompt binding, and validation for exactly one new intake or one explicitly approved atomic intake series. The domain delivery set consists of exactly the five canonical contract artifacts and the positive and negative evidence named by the intake.

**Ausgeschlossen / Excluded** sind die Ausfuehrung erzeugter Prompts, Update oder Delete bestehender Intakes, Produktimplementierung, Remote Writes ausserhalb des spaeteren autorisierten Feature-Closeouts, Admin-Bypass, Provider-Administration, Level-0-Arbeit, Preset-Promotion und der Start eines weiteren Lastenhefts. / Excluded are execution of generated prompts, update or delete of existing intakes, product implementation, remote writes outside the later authorized feature closeout, admin bypass, provider administration, Level-0 work, preset promotion, and starting another intake.

### Kanonische Vertragsartefakte / Canonical contract artifacts

Genau diese fuenf Pfade bilden den fachlichen Vertrag; die im akzeptierten Intake gebundene Preset-Basis ist `intake-authoring-governance 0.3.0`. Pfad-, Inhalt- oder Versionsdrift erzwingt eine erneute Bindungs- und Review-Pruefung. / Exactly these five paths form the domain contract; the preset baseline bound by the accepted intake is `intake-authoring-governance 0.3.0`. Path, content, or version drift requires renewed binding and review.
...
- **CR-004**: Statistik wird erst nach abgeschlossener Implementierungsphase oder Feature-Abschluss nach Methodik v2 fortgeschrieben; gemeinsame Agenten-Guidance wird durch diese Spezifikation nicht geaendert. / Statistics is updated only after a completed implementation phase or feature completion under methodology v2; this specification does not change shared agent guidance.
...
- **CR-007**: Die installierten Governance-Presets werden ohne zusaetzliche Autoritaetsableitung bewertet; nur Intake Authoring, der aktuelle autonome Lauf, Cross-Platform, A11Y, Security und die begrenzte Vertragsarchitektur sind fachlich anwendbar. / Installed governance presets are assessed without deriving extra authority; only intake authoring, the current autonomous run, cross-platform, accessibility, security, and the bounded contract architecture are materially applicable.
- **CR-008**: Genau eine Dokumentationsauswirkung gilt: `UpdateRequired`; Quelle, Owner, Leserpfad, Sprachstrategie, Plattformnachweis, Distribution und Re-Evaluation sind unten gebunden. / Exactly one documentation impact applies: `UpdateRequired`; source, owner, reader path, language strategy, platform evidence, distribution, and reevaluation are bound below.
...
- Der akzeptierte Intake bindet `intake-authoring-governance 0.3.0`. Eine nachgewiesene Preset-, Template-, Validator- oder Mindestversionsdrift wird nicht still uebernommen, sondern loest die im Intake geforderte Re-Evaluation aus. / The accepted intake binds `intake-authoring-governance 0.3.0`. Proven preset, template, validator, or minimum-version drift is not silently adopted and triggers the reevaluation required by the intake.
...
| Agent Parity Governance | N/A | Not Assessed | Keine gemeinsame Agenten-Guidance, kein Projekt-Template und keine Constitution wird in diesem Feature geaendert. / No shared agent guidance, project template, or constitution changes. | AOC-Maintainer; kein Restrisiko; Re-Evaluation bei Aenderung von `AGENTS.md`, `CLAUDE.md`, `GEMINI.md`, `.github/copilot-instructions.md`, `.github/agents/copilot-instructions.md` oder `.specify/templates/`. |
...
Genau eine Entscheidung gilt: **`UpdateRequired`**. / Exactly one decision applies: **`UpdateRequired`**.

- **Kanonische Quelle und Owner / Canonical source and owner**: META-LH-03 und die genau fuenf kanonischen Vertragsartefakte; Owner ist der AOC-Maintainer. / META-LH-03 and exactly five canonical contract artifacts; the AOC maintainer is owner.
- **Zielgruppen und Leserpfad / Audiences and reader path**: Lernende, erfahrene Fachkraefte, Maintainer und Reviewer lesen Zweck, Voraussetzungen, Begriffe, Scope, Vertrag, Pruefbeispiele, Nicht-Autoritaet und genau eine sichere naechste Aktion in dieser Reihenfolge. / Apprentices, professionals, maintainers, and reviewers follow purpose, prerequisites, terms, scope, contract, test examples, non-authority, and exactly one safe next action in that order.
- **Dokumentfamilien und Navigation / Document families and navigation**: Feature-Spezifikation und Requirements-Checkliste werden jetzt aktualisiert; spaetere Plan-/Task-Artefakte duerfen nur die fuenf Vertragsartefakte und ihre Evidence strukturieren. Beschreibende relative Links erhalten den Leserpfad. / The feature spec and requirements checklist are updated now; later artifacts may structure only the five contract artifacts and their evidence. Descriptive relative links preserve navigation.
- **Sprache und Partner / Language and partner**: kurze Dokumente bleiben inline Deutsch zuerst und Englisch danach auf CEFR B2; keine separate Partnerdatei. / Short documents stay inline German first and English second at CEFR B2; no separate companion file.
- **Plattform- und Beispielnachweis / Platform and example evidence**: Bash-/PowerShell-Paritaet, drei Fixture-Suites, 14 Receipts, semantisches Review und vollstaendiger Gitleaks-Scan bilden die spaetere Evidence. / Bash/PowerShell parity, three fixture suites, fourteen receipts, semantic review, and full Gitleaks scan form later evidence.
- **Distribution und Home Sync / Distribution and Home sync**: repository-lokale Level-2-Quelle; kein Home-Sync, keine Level-0-Propagation und keine Preset-Promotion. / Repository-local Level-2 source; no Home sync, Level-0 propagation, or preset promotion.
- **Statistik / Statistics**: Die Specify-Phase loest keine Fortschreibung aus. Nach abgeschlossener Implementierungsphase oder Feature-Abschluss wird Methodik v2 ueber den kanonischen Renderer angewendet. / Specify does not trigger a statistics update. Methodology v2 applies after implementation phase or feature completion through the canonical renderer.
- **Agenten-Guidance / Agent guidance**: Das Pruefergebnis innerhalb `UpdateRequired` lautet, dass sich keine gemeinsame Agentenregel aendert. Re-Evaluation bei einer Aenderung der fuenf gepflegten Agentenflaechen oder Projekt-Templates. / The checked result within `UpdateRequired` is that no shared agent rule changes. Reevaluate if maintained agent surfaces or project templates change.
- **Evidence und Re-Evaluation / Evidence and reevaluation**: `specs/003-authoring-contract/spec.md`, `specs/003-authoring-contract/checklists/requirements.md` und der strukturierte Runner-Phasenbeleg sind aktuelle Evidence. Intake-, Review-, Receipt-, Vertrags-, Preset-, Plattform-, Security-, Authority- oder Hash-Drift erzwingt erneute Bewertung. / The spec, checklist, and structured runner phase result are current evidence. Any named drift requires reevaluation.

## Aktuelle Phasengrenze / Current Phase Boundary

Diese Phase schreibt nur `spec.md`, `checklists/requirements.md`, `.specify/feature.json` und den vom Runner erzeugten strukturierten Phasenbeleg. Sie aendert keine Intakes, Receipts, Reviews, Presets, Baseline, Run-State-Datei oder vorhandene Evidence, startet keine naechste Phase und fuehrt keinen Git- oder Remote-Schritt aus. / This phase writes only the spec, requirements checklist, feature pointer, and runner-produced structured phase result. It changes no intake, receipt, review, preset, baseline, run-state file, or existing evidence, starts no next phase, and performs no Git or remote action.
