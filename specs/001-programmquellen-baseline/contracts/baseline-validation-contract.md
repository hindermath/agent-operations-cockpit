...
| `terminal-rename` | Die Feature-Lineage ab der akzeptierten Base enthaelt exakt einen byteidentischen `R100`-Lifecycle-Commit vom Original- zum branch-gestempelten Archivpfad, den exakten Co-Author-Trailer und keine Stage-Reste. Danach ist nur ein generierter Statistik-Synchronisationscommit zulaessig; dessen Head wird unveraendert reviewt. / The feature lineage contains exactly one byte-identical R100 lifecycle commit; only the generated statistics synchronization may follow before the reviewed head. |
...
`candidate-paths.json` ist nur die maximale Feature-Grenze und nennt Lifecycle-Datensatz, beide Rename-Pfade und `causal-closeout-evidence.json` bereits als Pending-Anker. Die normale Kandidatenmenge wird zweipassig eingefroren und exakt gestaged. Nach dem normalen Kandidaten-Commit darf ausschliesslich `scripts/rename-lastenheft.sh` den Originalpfad als byteidentischen Archivpfad committen; danach darf genau `docs/project-statistics.md` mit dem vorhandenen Renderer synchronisiert und separat committed werden. / The maximum allowlist includes the lifecycle transition and permits exactly one generated statistics synchronization after the rename.

Der normale Kandidat wird zuerst committed. Der Rename ist die letzte fachliche Polish-Aktion und der letzte Intake-mutierende Commit; `terminal-rename` prueft dessen exakten `R100`-Inhalt in der Feature-Lineage. Anschliessend rendert genau ein separater Commit ausschliesslich `docs/project-statistics.md`, damit die Methodik-v2-Quelle den Rename-Commit bindet. Dieser Statistik-Head wird gepusht, reviewt, exact-head validiert und gemergt; danach ist jede Feature-Head-Mutation verboten. Nach Fast-forward-Sync entsteht genau `codex/001-programmquellen-baseline-closeout` von `main`. Dessen Write-Allowlist ist exakt `tasks.md`, `autonomous-run-state.json` und `causal-closeout-evidence.json`. Vor seinem einzigen Commit werden archivbewusste Finalvalidierung und beide Re-Reviews abgeschlossen; `causal-closeout` prueft Stage, 66/66, Hash, terminalen State und die Nicht-Selbstreferenz. / The rename is the final intake mutation; one generated statistics-only commit forms the immutable reviewed head before the exact three-path closeout.
