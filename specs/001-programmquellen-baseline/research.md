...
**Entscheidung / Decision**: Der normale Kandidat enthaelt einen vorbenannten `Pending`-Anker. Auf seinen Commit folgt der constitutionelle terminale Rename als letzter Intake-mutierender Commit. Danach synchronisiert genau ein separater, generierter Commit ausschliesslich `docs/project-statistics.md`, weil Methodik v2 den letzten nicht ausgeschlossenen Rename-Commit als Quelle bindet. Dieser Statistik-Head wird gepusht, reviewt, exact-head validiert und gemergt. Nach Fast-forward-Sync entsteht genau `codex/001-programmquellen-baseline-closeout` von synchronisiertem `main`. Sein einziger Commit aendert exakt Tasks, schema-1.1-Run-State und Causal Evidence, wird durch `causal-closeout` validiert und beendet T066 lokal. Spaetere Push-/Review-/Check-/Merge-/Cleanup-Fakten dieses Closeout-Commits werden extern berichtet und nicht selbstreferenziell in ihn geschrieben. / The rename is the final intake mutation; one statistics-only commit forms the immutable reviewed feature head before the exact three-path closeout.
...
- Tasks und State nach dem terminalen Rename auf dem Feature-Branch committen: verworfen. Ausschliesslich der generierte Statistik-Synchronisationscommit ist erlaubt; Tasks und State bleiben dem spaeteren Drei-Pfad-Closeout vorbehalten. / Tasks and state remain excluded; only the statistics synchronization may follow the rename.
