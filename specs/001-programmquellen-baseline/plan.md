...
11. Aktuelle Autoritaet fail-closed pruefen und den normalen Kandidaten committen. Danach bei sauberer Feature-Schreibflaeche `scripts/rename-lastenheft.sh` ausfuehren; dies ist die letzte fachliche Polish-Aktion und erzeugt die letzte Intake-Mutation. `terminal-rename` beweist genau einen byteidentischen R100-Lifecycle-Commit, den exakten Trailer und keine Stage-Reste. Anschliessend erzeugt der vorhandene Renderer genau einen separaten `docs/project-statistics.md`-Commit; erst dieser Head ist unveraenderlich und reviewfaehig. / Commit the normal candidate, perform the final intake rename, and then create the sole generated statistics-only reviewed head.
