...
Beim damaligen Authoring galt als historischer Snapshot: RAW-08 war der einzige
`Eligible`-Kandidat und RAW-09 blieb bis zum Abschluss seines Vorgängers
`Blocked`. Dieser Snapshot ist keine aktuelle Lifecycle-Quelle. Der aktuelle
kanonische Zustand steht ausschließlich im
[`manifest.json`](../../../specs/intake-series/aoc-phase-2/manifest.json) und
in der [`order.md`](../series/order.md). IAD901 und IAD902 sind beantwortet;
der gültige RAW-08-Handoff bleibt bindende Voraussetzung. / *At authoring time,
the historical snapshot recorded RAW-08 as the sole Eligible candidate and
RAW-09 as Blocked until its predecessor completed. This snapshot is not a
current lifecycle source. Only the linked manifest and order document define
the current canonical state. IAD901 and IAD902 are answered; the valid RAW-08
handoff remains a binding prerequisite.*
