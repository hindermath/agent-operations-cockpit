...
1. Beim damaligen Authoring galt als historischer Snapshot: RAW-05 und RAW-06
   waren `Completed`, RAW-08 war der einzige deklarierte `Eligible`-Kandidat
   und RAW-09 war blockiert. Dieser Snapshot ist keine aktuelle
   Lifecycle-Quelle. Der aktuelle kanonische Zustand steht ausschließlich im
   [`manifest.json`](../../../specs/intake-series/aoc-phase-2/manifest.json)
   und in der [`order.md`](../series/order.md). / *At authoring time, the
   historical snapshot recorded RAW-05 and RAW-06 as Completed, RAW-08 as the
   sole Eligible candidate, and RAW-09 as blocked. This snapshot is not a
   current lifecycle source. Only the linked manifest and order document
   define the current canonical state.*
2. RAW-05 und RAW-06 liefern weiterhin die bindenden Vorgängerverträge. /
   *RAW-05 and RAW-06 continue to provide the binding predecessor contracts.*
...
4. Der gültige RAW-08-Handoff bleibt eine bindende Voraussetzung für RAW-09. /
   *The valid RAW-08 handoff remains a binding prerequisite for RAW-09.*
