...
Beim damaligen Authoring galt als historischer Snapshot: RAW-04 war
`Completed`, RAW-07 war `Blocked` und es gab keinen `Eligible`-Kandidaten.
...
`research-only`; `parallel-autonomous` beschreibt nur mögliche spätere
disjunkte Adapterläufe nach eingefrorenem Contract, Lab Evidence und separater
Authority. / *At authoring time, the historical snapshot recorded RAW-04 as
Completed, RAW-07 as Blocked, and no Eligible candidate. This snapshot is not
a current lifecycle source. Only the linked manifest and order document define
the current canonical state. Research-only remains the allowed mode;
