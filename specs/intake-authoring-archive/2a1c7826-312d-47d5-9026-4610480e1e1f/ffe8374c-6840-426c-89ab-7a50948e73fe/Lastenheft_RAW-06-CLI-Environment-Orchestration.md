...
RAW-05 ist im aktuellen Series-Lifecycle `Completed`. RAW-06 bleibt dort
...
`single-autonomous` beschreibt nur einen möglichen späteren, separat
autorisierten Lauf. / *RAW-05 is Completed. RAW-06 remains Blocked until this
update has a current validated Ready Single review and a later explicit Series
update is authorised. No target is currently Eligible. The allowed mode stays
research-only; single-autonomous describes only a possible later, separately
authorised run.*
