...
| Statistik / statistics | `docs/project-statistics.md`, historische T055-/T079-Evidence und temporaere neue Final-Head-Bindung ausserhalb des Repositorys / historical evidence and temporary final-head binding | `8f395f8` und `0b0808c` bleiben verbrauchte unveraenderliche Historie. Nach genau einem normalen Ubuntu-Kardinalitaets-Follow-up auf `0b0808c` wird der unveraenderte Renderer geprueft. Nur bei Drift entsteht hoechstens ein neuer `statistics-head-sync`-Commit ausschliesslich aus dem Ledger; Methodik v2 schliesst ihn aus. Beide realen Peers muessen auf dem finalen Head `CURRENT`/`0` melden, und jede betroffene Head-Evidence wird dorthin neu gebunden. / The published repair and ledger head remain immutable; one follow-up and at most one renderer-proven ledger-only sync lead to the final bound head. |
...
requirements/baseline/portfolio-ownership.md
  ...
  -> genau ein normaler ubuntu-bash-cardinality-followup auf 0b0808c
...
Kein paralleler Worker darf einen dieser Pfade gleichzeitig schreiben. `8f395f8` und `0b0808c` bleiben verbrauchte Evidence ohne Wiederverwendungsautoritaet. Die optionale neue Synchronisation konsumiert ausschliesslich den gefrorenen normalen Follow-up-Head; Runtime-Artefakte, Caches, fremde Pfade und das Ledger selbst sind kein Methodikinput. Ein neuer Statistikcommit ist nur bei Drift zulaessig, enthaelt nur das Ledger und wird von Methodik v2 ausgeschlossen. Jede spaetere Head-Mutation invalidiert die Bindung und verlangt eine neue explizite Transaktionsreview. AEPS-Derivationen werden nur bei echtem Finding atomar aktualisiert; es bleibt genau eine bestehende `UpdateRequired`-Dokumentationsentscheidung. / *The final follow-up head alone drives any renderer-proven synchronization; consumed history grants no reuse.*

## Finale Ubuntu-Bash-Kardinalitaetsreparatur / Final Ubuntu Bash cardinality remediation

`implement-resume-8` ist auf PR #29 und exaktem Head `0b0808c56be649d088b397c6a88463ff5f52edb6` wahrheitsgemaess `Blocked`. Exakt 16/18 Checks bestehen; beide Restfehler sind derselbe Ubuntu-Defekt, bei dem `/usr/bin/bash` und `/bin/bash` als zwei `ApplicationInfo`-Treffer ein Array erzeugen. `8f395f8` und `0b0808c` bleiben unveraenderliche historische Evidence. Die spaetere Implementierung besitzt genau eine neue Follow-up-Transaktion mit diesen Mindestregeln: / *Implement-resume-8 is blocked only on duplicate-alias array cardinality. One final bounded follow-up is permitted.*

1. Implementierend darf nur `.github/workflows/powershell-analysis.yml` geaendert werden. Der Nicht-Windows-Zweig ordnet Bash-`ApplicationInfo`-Kandidaten deterministisch und reduziert sie vor `Source` auf genau einen Eintrag. / Only the workflow implementation changes.
2. Null Kandidaten scheitern fail-closed. Ein Kandidat erzeugt genau einen nichtleeren absolut qualifizierten Pfad. Mehrere Alias-Kandidaten erzeugen deterministisch genau einen Pfad; Arrays oder verbundene Pseudopfade duerfen die Invocation nie erreichen. / Prove zero, one, and duplicate-alias behaviour.
3. Der vorhandene sichere Argumentarray-Aufruf bleibt bestehen. Windows-Git-for-Windows-Validierung und WSL-Ablehnung bleiben unveraendert. / Preserve the safe arguments and Windows branch.
4. Nur falls ein bestehender akzeptierter Evidence-Pfad es erfordert, darf minimale fokussierte Vertrags-/Test-/Evidence-Wortlautbindung fuer diese drei Faelle hinzugefuegt werden. Produkt-, Provider-, Feature- oder Governance-Scope darf nicht wachsen. / Add only minimal existing-path proof if required.
5. Genau ein normaler Follow-up-Commit auf `0b0808c` ist erlaubt. Kein Amend, Force-Push oder History-Rewrite. Nur wenn der unveraenderte Methodik-v2-Renderer Drift meldet, darf hoechstens ein weiterer Commit nur `docs/project-statistics.md` synchronisieren. / One normal commit and at most one renderer-proven ledger-only sync.
6. PR #29 wird an den resultierenden unveraenderlichen finalen Head gebunden. Alle 18 Checks, exakte Ubuntu-/macOS-/`windows-2022`-Evidence und Review-Konvergenz laufen neu. Technischer Fehler ist nicht bypassbar. / Rebind and rerun all head-bound gates.
7. Erst danach werden T080 bis T093 fortgesetzt; beide terminalen Pfade enden mit No-next-run nach META-LH-02. / Continue the accepted closeout only after green and start no next run.
...
| `PowerShell Static Analysis / PSScriptAnalyzer` | Ubuntu, Windows, macOS | Auf Nicht-Windows deterministische skalare Bash-Auswahl mit null/eins/mehrere-Alias-Proof; vorhandener exakter Event-Head-/Branch-/Targetvertrag und der unveraenderte Windows-Git-for-Windows-/WSL-Vertrag bleiben bindend. Workflow-/Job-/Runner-/Head-/Log-/Command-/Exit-Evidence fuer alle drei Plattformen ist Pflicht. / Add deterministic scalar Bash selection while preserving all accepted exact-head and Windows controls. |
...
- Enger Bypass: auf PR #29 wegen der zwei Ubuntu-Auspraegungen des technischen Kardinalitaetsfehlers ausdruecklich unzulaessig. Erst nach neuem Head, allen 18 erfolgreichen/regelkonform uebersprungenen Checks, vollstaendiger Security-/Plattform-/Review-Evidence, null handlungsrelevanten Threads und genau einer verbleibenden Approval-/Ruleset-Policy darf `PO-N01` neu bewertet werden. / The remaining technical failures prohibit bypass until complete green reconvergence leaves only one policy blocker.
