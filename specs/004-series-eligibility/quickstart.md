...
Der bestehende Resolver belegt eine ausführbare Bash, aber prüft keine Mindestversion. Sein unveränderter Regressionstest darf deshalb nicht behaupten, Bash 3.2 zurückzuweisen. Der neue Feature-004-Schritt prüft zusätzlich die tatsächliche Major-Version >= 5. Auf macOS darf er eine bereits installierte Bash 5+ aus den aufgelösten Kandidaten wählen und ihren Ordner für seine Kindprozesse in PATH voranstellen; kein Installationsfallback. Auf Windows bleibt ausschließlich die geprüfte Git-for-Windows-Datei zulässig. Fehlt die passende Version, blockiert die native Matrix. Die Versionsergänzung bleibt im benannten Workflow, die bestehende Feature-003-Policy bleibt unverändert. / *The existing resolver proves executability, not a minimum version; its unchanged regression test cannot claim to reject Bash 3.2. The added Feature-004 step checks major version >= 5, can select an already installed macOS Bash 5+ and prepend its directory for child processes, and permits only checked Git Bash on Windows. No installation fallback; absence blocks the matrix. Keep the additional check in the named workflow without changing Feature-003 policy.*
